...
- **`token_hash` stores `sha256(rawToken)` only** (lowercase hex, `crypto.subtle`). The raw token is 24 bytes from `crypto.getRandomValues` → URL-safe base64 (32 chars, no padding, ≥128-bit entropy), embedded in the clean `/share/<token>` URL (BrowserRouter; `share.ts:240`) and returned **exactly once** at creation; lookups are by hash. Owner list/revoke never see the raw token — they key off `ShareLinkMeta.id`, a **24-hex-char prefix** of the hash (enough to revoke/dedupe, useless as a credential).
