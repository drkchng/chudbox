...
6. *(Optional, deferred)* The one free Cloudflare **rate-limit rule** on `^/api/(auth|share)/`. Image Transformations stay off (the image pipeline doesn't depend on them).
