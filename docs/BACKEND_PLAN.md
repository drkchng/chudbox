...
> **M3 built 2026-06-17 — how the options above resolved.** The **binding-proxy** branch shipped, **not** presigned PUT: the client multipart-POSTs the downscaled bytes to same-origin `POST /api/uploads`, which derives the `u/<userId>/…` key prefix from the validated session and streams to `env.BUCKET.put()` — no S3 API token, no `aws4fetch`, no bucket CORS (verified research fact #2). Serving is **direct** R2 object reads via `GET /img/<key>` (owner-only: the key embeds the owner's userId, decoded + checked against the session before serving; `Cache-Control: private, immutable`). Cloudflare Image Transformations (`/cdn-cgi/image`) is **deliberately not a dependency** — direct serving is the correctness path; transforms are a future, optional enhancement (fact #1). The canvas encoder is **feature-detected for WebP**: it attempts WebP and keeps it only if the produced `Blob.type === 'image/webp'`, otherwise re-encodes JPEG (Safari/iOS silently ignores the WebP request on `canvas.toBlob` — fact #4); the **actually-produced** format drives the `.webp`/`.jpg` key extension, never a hardcoded `.webp`. Image quality/archival is **tiered by plan through a single `ImagePolicy` seam** (`packages/shared/src/imagePolicy.ts`): only the **FREE** tier exists today — 1600px long edge, q≈0.78, **downscaled-only** (no pristine original archived); a paid tier is just a different `ImagePolicy` constant read through the same seam — **no** subscription/entitlement/billing/plan-detection system was built (unplanned scope). **Delete-on-replace/tombstone exists server-side** (`POST /api/uploads/delete`, ownership-checked + idempotent batch delete) — though the client's delete-on-delete hook (`handleDeletedPhotos`) currently calls `DELETE /img/<r2Key>` rather than that endpoint and swallows errors, so client-driven deletes do not yet reach it (a wiring fix, not a redesign). Still **deferred** (documented gaps, Risk #8): a periodic reconciliation sweep for orphans the hook misses, and an account-deletion `u/<uid>/` prefix purge.
...
> **M4 built 2026-06-17 — decisions resolved.** Shipped in `apps/api/src/routes/share.ts` (owner + public routes), DO `getCarSnapshot`/`resolveSharePhotoKey` RPCs (`apps/api/src/durable/GarageDO.ts`), and the allowlist curator `packages/shared/src/publicSnapshot.ts` (full DDL + field list in `db/schema.md`). **(1) Expiry = optional per link** — default is **no expiry** (`expires_at` NULL); the owner share dialog offers an optional future date (epoch seconds, re-checked against the insert clock so the DB CHECK always holds); **revoke is always available + idempotent**. **(2) Snapshot = curated build showcase via an explicit allowlist (deny-by-default)** — a field is exposed IFF named, so anything new stays private automatically. Exposes only: car year/make/model/trim/color/nickname/mileageRaw(+miles)/status/purchase&saleDate/createdAt + cover; photos (photoId/caption/dims); mods (name/category/description/installedDate/link/addedAt); maintenance (service/date/mileage/nextDue/createdAt); display settings (themeId/customAccent/distanceUnit). **Withheld entirely:** every money amount + `*Currency`, every `shop`, every `notes`, `tradeFor`, `salePrice`, raw `r2Key`/base64 `dataUrl`, and the whole **wishlist / issues / todos** tables (`settings.currency` is read DO-side but never reaches the snapshot — no money is shown). Curation runs **DO-side** before any private cell crosses the RPC boundary; exhaustively asserted in `publicSnapshot.test.ts`. **Token-scoped images:** the snapshot carries **photoIds only** — the viewer builds `/api/share/:token/img/:photoId` (`shareImgPath`), which derives owner/car/`r2Key` server-side from the validated row (returning the key only for a photo under that car that already has an `r2Key`), so the raw key and owner-only `/img` path never leave the server. Token is a bearer credential: 24 bytes from `crypto.getRandomValues` → URL-safe base64, only `sha256(token)` stored, raw token shown **once**; owner list/revoke key off `ShareLinkMeta.id` (a 24-char hash prefix). Create is **DO-check-then-D1-insert**; **lazy-revoke → 410** when a later snapshot read finds the car gone. Public viewer is a dedicated read-only `ShareCarView` reusing `CarHero`/tab idioms (not the editable `CarProfile`). *(Edge cache is the `Cache-Control: public, max-age=60` header on the snapshot + image responses; no separate share-route rate limiter beyond that shipped yet.)*
...
| **M3** | **Images → R2.** Client downscale → presigned PUT → `r2Key` in row → display via `/img`; migrate parked base64 photos to R2. *(Built 2026-06-17 — shipped via **binding-proxy upload, not presigned**; WebP feature-detected with JPEG fallback; FREE `ImagePolicy` tier seam. See the Image-pipeline "M3 built" note.)* | `apps/api/routes/uploads.ts`,`img.ts`; `apps/web` `src/components/tabs/PhotosTab.tsx` (FileReader→compress+PUT) + `<img>` swap in `CarCard.tsx`,`CarProfile.tsx`; R2 bucket+CORS; `photos` schema (`r2Key`) | Presigned key-prefix scoping; CORS; EXIF; migrating large local blobs; R2 orphans; transform billing |
| **M4** | **Share-links.** Unguessable, hashed-at-rest read-only public car page (no account). *(Built 2026-06-17 — optional per-link expiry (default none) + always-available revoke; curated allowlist showcase with money/shop/notes/`tradeFor`/`salePrice`/wishlist/issues/todos withheld; token-scoped `/api/share/:token/img/:photoId` derived server-side. See the Share-links "M4 built" note.)* | `apps/api/routes/share.ts`; D1 `share_links`; DO `getCarSnapshot`/`resolveSharePhotoKey` RPCs; `packages/shared/publicSnapshot.ts`; web `SharePage`/`ShareCarView` + `ShareDialog` | Abuse/rate-limit (per-colo cache); token-as-credential; DO-check-then-D1 ordering; lazy-revoke; image exposure scope |
| **M5** | **Mobile-ready hardening.** Lock `@chudbox/shared` RN-safety; add `persister-expo-sqlite`; stub `apps/mobile` Expo app reusing `createGarageStore()`. *(M5 2026-06-17: scope reduced — `packages/shared` locked & verified RN-safe + `docs/MOBILE.md` reuse guide; the `apps/mobile` Expo stub and RN toolchain are DEFERRED to a dev-machine step by user decision. See `docs/MOBILE.md`.)* | `packages/shared` (no Node/DOM deps); `apps/mobile` Expo skeleton; Metro config | `crypto.randomUUID` in RN; Metro resolution (improved on RN ≥0.79; tsup output + explicit import paths as backstop) |
...
> **Status 2026-06-17:** all locally-buildable milestones (**M0–M5-reduced**) are complete. Remaining work is the deploy-prep kickoff items below (custom domain, Cloudflare account + real D1/R2/DO provisioning, Resend, secrets, CI) plus the live `apps/mobile` build (deferred to a dev-machine step — see `docs/MOBILE.md`).
